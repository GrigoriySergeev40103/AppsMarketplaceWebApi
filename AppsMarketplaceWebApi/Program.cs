	...
	Events = new()
	{
		OnBeforeCreateAsync = async ctx =>
		{
			...
			[
				...
				( "extension",   ("App's extension is a zero length string after trimming"			, "App's extension must be specified") )
			];
		...
		OnFileCompleteAsync = async eventContext =>
		{
			...
			toAdd.Name				 = metadata["filename"].GetString(Encoding.UTF8).Trim();
			...
			toAdd.Extension			 = metadata["extension"].GetString(Encoding.UTF8).Trim();
...
app.MapTus("/append", async (httpContext) => new()
{
	...
	Events = new()
	{
		...
		OnFileCompleteAsync = async eventContext =>
		{
			...
			App? toAppend = await dbContext.Apps.AsNoTracking().SingleOrDefaultAsync(a => a.AppId == file.Id);
			if (toAppend == null)
			{
				Console.WriteLine("ERROR: failed to find file to append incoming gile to. Discarding file...");
				await DiscardFile();
				return;
			}
			...
			AppFile appFile = new()
			{
				...
				Filename = toAppend.Name,
				Path = tusStorePath + $"/{toAppend.AppId}"
			};
